...
//						break;
			...
			for each(var menuItem:XML in menu){
				...
				if(obj != null){
					...
					if(menuItem.@data==i18n._('GLOBAL_EDIT')){						
						menuItem.@enabled = RightService.canUpdate(entity,currentUser.id==obj.OwnerId);
					}
					if (menuItem.@data==i18n._('GLOBAL_DELETE')){						
						menuItem.@enabled = RightService.canDelete(entity,currentUser.id==obj.OwnerId);
					}
		...
		*/
			...
			calendar.addEventListener(MouseEvent.RIGHT_CLICK, 
				function showMenu(e:MouseEvent):void{
					...
					if(Database.preferencesDao.getBooleanValue(PreferencesDAO.ENABLE_BUTTON_ACTIVITY_CREATE_CALL)){
						var newCall:XML=new XML('<menuitem label="'+i18n._('GLOBAL_NEW_CALL')+'" icon="activityCallIcon"/>');
						rootMenu.appendChild(newCall);
					}
					if(selectedItem != null){
						var editMenu:XML = new XML('<menuitem label="'+i18n._('GLOBAL_EDIT')+'" icon = "editIcon" />');
		...
		public static function getContextMenuMiniRelationGrid(detail:Detail, grid:DataGrid, handler:Function):void{
			var rootMenu:XML = <root />;
		...
		public static function getContextMenuMiniDetail(detail:Detail, grid:DataGrid, openDetail:Function, objectSQLQuery:Object,refreshGrid:Function):void {
			
			// Feature #58
			var enabled:Boolean = GUIUtils.isEnableSR(detail);
			
			var rootMenu:XML = <root />;
			var addMenu:XML = new XML('<menuitem label="'+i18n._('GLOBAL_ADD')+'" icon="@Embed(\'/assets/accept.png\')" enabled="'+enabled+'" />');
			var updateMenu:XML = new XML('<menuitem label="'+i18n._('GLOBAL_UPDATE')+'" icon="@Embed(\'/assets/edit.png\')" enabled="'+enabled+'" />');
			var deleteMenu:XML = new XML('<menuitem label="'+i18n._('GLOBAL_DELETE')+'" icon="@Embed(\'/assets/delete.png\')" enabled="'+enabled+'" />');
			rootMenu.appendChild(addMenu);
