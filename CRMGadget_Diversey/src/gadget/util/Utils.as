package gadget.util {
	...
	public class Utils {
		...
		public static function getFieldNameFromIntegrationTag(entity:String, integrationName:String):String {
			...
			var fieldMgmtNames:Array = Database.fieldManagementServiceDao.readAll(entity);
		...
		public static function doJoninFieldValue(entity:String,filterQuery:String,fieldName:String):String{
			...
			var daoFieldName:String = getFieldNameFromIntegrationTag(entity,fieldName);
			if(daoFieldName != null){
				var records:ArrayCollection = dao.findAll(new ArrayCollection([{element_name:daoFieldName}]), filterQuery);
				if(records.length>0){
					return records[0][daoFieldName] ;
				}
			}
			return "";
		}
			...
			for each (var tmpField:Object in fields) { 
				if (tmpField.custom == null) {
					...
					for each(var fieldManagement:Object in fieldsManagement){
						if(SupportService.match(fieldManagement.Name,fieldName ) && fieldManagement.DefaultValue != null
							&& fieldManagement.DefaultValue != ''){
							var defaultValue:String = fieldManagement.DefaultValue;
							
							if(afterSave && defaultValue.indexOf("JoinFieldValue")==-1){
								continue;
							}
					...
					if(userData!=null && !afterSave){	
						if(entity == Database.activityDao.entity && enityObject['DueDate'] == null){
		...
		public static function doEvaluateForFilter(criteria:Object,entity:String):String{
			...
			if(criteria.param != null && criteria.param != ""){
				var param:String =  Evaluator.evaluate(criteria.param, Database.allUsersDao.ownerUser(), entity, null, null, PicklistService.getValue,PicklistService.getId,Database.preferencesDao.getValue,true,null,getFieldNameFromIntegrationTag);
		...
		public static function doEvaluate(defaultValue:String,userData:Object,entity:String,element_name:String,entityOject:Object,sqlList:ArrayCollection):String{
			...
			return Evaluator.evaluate(defaultValue, userData, entity, element_name, entityOject, PicklistService.getValue,PicklistService.getId,null,false,Utils.doJoninFieldValue,getFieldNameFromIntegrationTag,sqlList);
		...
		public static function deleteChild(dest:Object,entity:String):void{
			...
			}else if(entity==Database.customObject3Dao.entity){
				//will be delet product by plan
				if(!StringUtils.isEmpty(dest.ProductId)){
					Database.productDao.deleteByOracleId(dest.ProductId);
				}
		...
		public static function styleFunction(item:Object, column:AdvancedDataGridColumn):Object {
			...
			if (item.modified == "UPD") return { color:0x0000FF, fontWeight:"bold" };
			if (item.modified == "ERR") return { color:0xFF0000, fontWeight:"bold" };
			return {};
		}
		...
		public static function calculateStartTime(type:Number):Number{			
			...
			if(type == TransactionDAO.ONE_YEAR_TYPE){				
					return calculateDate(-1,currentDate,"fullYear").getTime();
			}else if(type == TransactionDAO.SIX_MONTH_TYPE){
				return calculateDate(-6,currentDate,"month").getTime();
				
			}else if(type == TransactionDAO.THREE_MONTH_TYPE){
				return calculateDate(-3,currentDate,"month").getTime();
				
			}else if(type == TransactionDAO.ONE_MONTH_TYPE){
				return calculateDate(-1,currentDate,"month").getTime();
				
			}else if(type == TransactionDAO.SEVEN_DAY_TYPE){
				return calculateDate(-7,currentDate,"date").getTime();				
			}
		...
		public static function isSRHasAttPdf(srObj:Object, entity:String):Boolean{
			if(UserService.DIVERSEY == UserService.getCustomerId() && entity == Database.serviceDao.entity){
				if( srObj.CustomPickList11=="TECO"){
					//var isHasPdfAtt:Boolean = Database.serviceDao.isHasPdfAtt(srObj);
					
					return isHasAttPdf(srObj);
					
				}
			}
			return true;
		}
		public static function isHasAttPdf(srObj:Object):Boolean{
			var isHasPdfAtt:Boolean = Database.serviceDao.isHasPdfAtt(srObj);
			return isHasPdfAtt;
		}
		...
		public static function upload(file:File, entity:String, gadget_id:String, fnRefresh:Function=null, fnModifiedItem:Function=null):void {
		...
		*/		
		...
		public static function writeUTFBytes(file:File, data:String):void {
			try {
		...
		public static function getAllFilters(entity:String):ArrayCollection{
			var userOwner:Object = userOwner = Database.allUsersDao.ownerUser();
						...
						if (displayValue == null || displayValue=="") {
							...
							if((parseColumn.ColumnName as String).indexOf("CustomMultiSelectPickList") != -1 && record[parseColumn.ColumnName]) {
								...
								for each(var data:String in record[parseColumn.ColumnName].toString().split(str_split)) {
									displayValue += i==0 ? piclistServiceAll[parseColumn.ColumnName + "/" + StringUtil.trim(data)] : "," + piclistServiceAll[parseColumn.ColumnName + "/" + StringUtil.trim(data)];
									i++;
								}
							}else {
								// bug #329
...
//					}					
		...
		public static function parseMultiPickListValue(entity:String,columnName:String,item:Object,languageCode:String):String{
		
			var i:int = 0; 
			var displayValue:String = "";
			var str_split:String = item[columnName].toString().indexOf(";") !=-1 ? ";" : ",";
			for each(var multipicklist:Object in PicklistService.getMultiPicklistByLanguage(entity, columnName,languageCode)) {
				for each(var data:String in  item[columnName].toString().split(str_split)) {
					if(multipicklist.data == StringUtil.trim(data)){ 
						displayValue += i==0 ? multipicklist.label : "," + multipicklist.label;
						i++;
					}
				}
			}
		...
		public static function closingSR(detailForm:Detail):void{
			Alert.show(i18n._("CANCEL_SERVICE_REQUEST"), "", Alert.YES|Alert.NO, detailForm, function(event:CloseEvent):void{
		...
		//#5938 CRO
		public static function getRation(byt:ByteArray):Number{
			var ration:Number = 1;
			if(byt.length > 300000 && byt.length < 600000){
				ration = 0.65;
			}else if(byt.length > 600000 && byt.length < 1000000){
				ration = 0.55;
			}else if(byt.length > 1000000 && byt.length < 2000000){
				ration = 0.45;
			}else if(byt.length > 2000000){
				ration = 0.30;
			}
			return ration;
		}
		//#5938 CRO
		public static function scaleBitmap(src: BitmapData, ratio: Number): BitmapData
		{
			var rect:Rectangle = new Rectangle(0,0,src.width* ratio,src.height * ratio);
		...
		//#5938 CRO
		public static function imageLoaded(loader:Loader,ration:Number):ByteArray {
			...
			var m: Matrix = new Matrix();   
			m.scale(1, 1);
			var myImage:Bitmap = new Bitmap(); 
			myBitmapData = new BitmapData(loader.width, loader.height); 
			myBitmapData.draw(loader,m); 
			var byt:ByteArray = com.adobe.images.PNGEncoder.encode(scaleBitmap(myBitmapData,ration));
			
			return byt;
		}
		
		public static function isMissingPDF():void{
			var filterQuery:String = "(OwnerId = (select id from user))AND(CustomPickList10='SUSP' OR CustomPickList10='AWPT' OR CustomPickList11='TECO') And gadget_id Not In (Select gadget_id from attachment where entity='Service Request' And gadget_id is not null and (filename like '%.pdf' or filename like '%.doc' or filename like '%.docx'))";
			var columns:ArrayCollection = new ArrayCollection(["Id"]);
			var records:ArrayCollection = Database.serviceDao.findAll(columns, filterQuery);
