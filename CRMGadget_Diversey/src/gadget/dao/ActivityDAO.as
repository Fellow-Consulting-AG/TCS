...
// DO NOT ENABLE UNTIL CALLBACK WORKS!	, "StartTime, EndTime", "EndTime"
			...
			stmtGetAppointment.text = "SELECT * FROM Activity WHERE (StartTime >= :startTime and StartTime <= :endTime) " + 
				" ORDER BY StartTime";
			//Get all activities by Today,ThisWeek,ThisMonth and Overdue 
			stmtFind = new SQLStatement();
			stmtFind.sqlConnection = sqlConnection;
			stmtFind.text = "SELECT '" + entity + "' gadget_type,* FROM Activity WHERE ((Status !=:status or Status is null or Status = '') and OwnerId=:ownerId and DueDate >= :startTime and DueDate < :endTime and Activity='Task') " + 
			...
			stmtFindUpcoming.text = "Select '" + entity + "' gadget_type,* from Activity Where (DueDate>=:startTime and DueDate< :endTime and OwnerId=:ownerId)" + 
				" ORDER BY Priority,DueDate Limit:limit";
			
			stmtFindDelegate = new SQLStatement();
			stmtFindDelegate.sqlConnection = sqlConnection;
			stmtFindDelegate.text = "Select '" + entity + "' gadget_type,* from Activity Where ( DelegatedById =:ownerId and Activity='Task') " + 
			...
			stmtGetAppointmentAccount.text = "SELECT act.*, a.PrimaryBillToStreetAddress, a.PrimaryBillToPostalCode, a.PrimaryBillToCity," +
					...
					" WHERE (act.StartTime >= :startTime and act.StartTime <= :endTime) ORDER BY act.StartTime";
