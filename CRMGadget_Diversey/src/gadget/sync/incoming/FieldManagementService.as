...
//		}
		private function getDataStr(field:XML, col:String):String {
			var tempField:String = col;
			if(col=='Name'){
				tempField = 'GenericIntegrationTag';
			}
			var tmp:XMLList = field.child(new QName(ns2.uri,tempField));
			if(tmp==null || tmp.length()==0){
				tmp = field.child(new QName(ns2.uri,col));//maybe used version 19
			}
			return tmp.length()==0 ? "" : tmp[0].toString();
		}
