	...
	public class JDIncomingPlant extends IncomingObject
	{
		...
		override protected function postImportRecord(tmpOb:Object):void{
			...
			if(tmpOb.IndexedBoolean0!='true'){
				...
				var productId:String=tmpOb['ProductId'];
				if(!StringUtils.isEmpty(tmpOb.ProductId)){
					if(i>0){
						searchProductSpec=searchProductSpec+"OR";
					}
					searchProductSpec=searchProductSpec+" [Id] = \'"+productId+'\' ';
					i=i+1;
				}
				
			}else{
				if(!StringUtils.isEmpty(tmpOb.ProductId)){
					//delete product when change consumable = true
					Database.productDao.deleteByOracleId(tmpOb.ProductId);
				}
			}
		}
