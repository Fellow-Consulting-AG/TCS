...
//				showCount();
		...
		//Bug #7167 CRO
		override protected function checkResponse(listObject:XML):void{
			//implement in the sub class
			//not found
			if(listObject.child(new QName(ns2.uri,entityIDns)).length() <= 0){
				var pdao:BaseDAO = Database.getDao(entityIDour);
				var item:Object = pdao.findByOracleId(pid);
				if(item != null){
					pdao.delete_(item);
					//delete child obj
					Utils.deleteChild(item,pdao.entity);
					Utils.removeRelation(item,pdao.entity,false);
				}else{
					//delete attachment no parent
					Database.attachmentDao.deleteByGadgetId(pid);
				}
			}
		}
