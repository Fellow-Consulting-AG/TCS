...
//		override protected function checkSRRequire(srObj:Object):Boolean{
//			if(UserService.DIVERSEY == UserService.getCustomerId() && entity == Database.serviceDao.entity && Utils.isHasAttPdf(srObj)){
//				return false;
//			}
...
//			}
			...
			for(var i:int = 0; i < records.length; i++){
				var parentRec:Object = Database.getDao(entity).findByGadgetId(records[i].gadget_id);
				if(parentRec==null){
		...
		override protected function handleRequestFault(soapAction:String, request:XML, response:XML, faultString:String, xml_list:XMLList, event:IOErrorEvent):Boolean{
			if (faultString.indexOf("(SBL-DAT-00382)")>=0 ||
				...
				var parentRec:Object = Database.getDao(entity).findByGadgetId(records[0].gadget_id);
				var sodid:String  = DAOUtils.getOracleId(entity);
